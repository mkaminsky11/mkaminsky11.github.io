...
$(document).ready(function(){
    ...
    $("#contact>div>div").hover(function(){
      ...
      var interval = window.setInterval(function(){
        var deg = Number($(this).find(">.circle").attr("data-deg"));
        if(deg === 360){
          clearInterval(interval);
        }
        else{
          deg++;
         $(this).find(">.circle").attr("data-deg", deg);
          renderCircle(id,deg)
        }
      }, 100);
...
function renderCircle(id, deg){
  ...
  con.beginPath();
  con.arc(can.width/2, can.height/2, 25, -(Math.PI / 2), deg*(2 * Math.PI / 360) - Math.PI / 2, false);
  con.stroke();
}
function add(n){
